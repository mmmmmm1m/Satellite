...
int main()
{
	...
	// This section confirms if the user wants to find a circular orbit or an elliptical orbit
	cout << "\nIs the orbit a circle or ellipse? c/e\n";
	while (true)
	{
		cin >> result;
		if (result == 'c') // User wants a cirle, continues to circular orbit calculations
		{
			break;
		}

		if (result == 'e') // User wants an elliptical orbit, program makes those calculations here
		{
			cout << "\nDo you know the semi-major and semi-minor axes? y/n\n";
			cin >> result2;
			while (true)
			{
				if (result2 == 'y')
				{
					cout << "\nWhat is the semi-major axis in meters?\n";
					cin >> _satellite.semimajoraxis;
					cout << "\nWhat is the semi-minor axis in meters?\n";
	...
	while (true)
	{
		if (result == 'y')
		{
			cout << "\nHow many hours,minutes,seconds is a sidereal day on that planet?\n";
			std::string time;
			cin >> time;
